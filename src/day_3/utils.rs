...
pub fn get_fabric_claims() -> HashMap<i32, Rect> {
    ...
    fabric_claim_str.lines().for_each(|l| {
        id_rect_map.insert(get_id(l), Rect { size: get_rect_size(l), location: get_rect_location(l) });
    });
...
fn get_rect_location(claim: &str) -> Point {
    ...
    Point { x: location[0].parse().unwrap(), y: location[1].parse().unwrap() }
}
...
fn get_rect_size(claim: &str) -> Size {
    ...
    Size { w: size[0].parse().unwrap(), h: size[1].parse().unwrap() }
}
