    ...
    fn lookup_largest_total_power(
        ...
        for y in 1..=self.size.y - sub_grid_size.y + 1 {
            for x in 1..=self.size.x - sub_grid_size.x + 1 {
                ...
                if x > 1 && y > 1 {
                    power_level -=
                        power_table[self.get_array_2d_flat_index(table_x - sub_grid_size.x, table_y)];
                    power_level -=
                        power_table[self.get_array_2d_flat_index(table_x, table_y - sub_grid_size.y)];
                    power_level += power_table
                        [self.get_array_2d_flat_index(table_x - sub_grid_size.x, table_y - sub_grid_size.y)];
                } else if x > 1 {
                    power_level -=
                        power_table[self.get_array_2d_flat_index(table_x - sub_grid_size.x, table_y)];
                } else if y > 1 {
